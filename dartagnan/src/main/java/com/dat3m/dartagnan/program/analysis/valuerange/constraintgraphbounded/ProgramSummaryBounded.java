...
public class ProgramSummaryBounded {
	...
	public ProgramSummaryBounded(Program program, AliasAnalysis aliasAnalysis) {
	...
	private void insertRestrictionEventsForJump(Event jump) {
		...
		if (rhs instanceof IValue) {
			...
			RestrictionEvent restriction2 = new RestrictionEvent(this.minNewCid++, thread, (Register) rhs, (Register) lhs, COpBin.turnAround(cmpOp), jump);
		...
		if (rhs instanceof IValue) {
			RestrictionEvent restriction = new RestrictionEvent(this.minNewCid++, thread, (Register) lhs, ((IValue) rhs).getValue(), COpBin.negate(cmpOp), jump);
			this.insertAfterEvent(label, restriction);
		} else {
			RestrictionEvent restriction1 = new RestrictionEvent(this.minNewCid++, thread, (Register) lhs, (Register) rhs, COpBin.negate(cmpOp), jump);
			RestrictionEvent restriction2 = new RestrictionEvent(this.minNewCid++, thread, (Register) rhs, (Register) lhs, COpBin.negate(COpBin.turnAround(cmpOp)), jump);

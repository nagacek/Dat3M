...
public class RelRf extends Relation {
    ...
    public void initialize(RelationAnalysis ra, RelationAnalysis.SetBuffer buf, RelationAnalysis.SetObservable obs) {
        ...
        WmmAnalysis wmmAnalysis = ra.getTask().getAnalysisContext().get(WmmAnalysis.class);
        logger.info("Computing maxTupleSet for " + getName());
        TupleSet maxTupleSet = new TupleSet();

        List<Event> loadEvents = ra.getTask().getProgram().getCache().getEvents(FilterBasic.get(READ));
        List<Event> storeEvents = ra.getTask().getProgram().getCache().getEvents(FilterBasic.get(WRITE));
        ...
        if(wmmAnalysis.isLocallyConsistent()) {
            applyLocalConsistency(maxTupleSet,ra);
        }
        if(wmmAnalysis.doesRespectAtomicBlocks()) {
            atomicBlockOptimization(maxTupleSet,ra);
        }

        logger.info("new knowledge size for {}: 0/{}", getName(), maxTupleSet);
        buf.send(this,maxTupleSet,new TupleSet());
    }

    private void applyLocalConsistency(TupleSet maxTupleSet, RelationAnalysis ra) {
        // Remove future reads
        ...
        for (Event r : ra.getTask().getProgram().getCache().getEvents(FilterBasic.get(READ))) {
    ...
    private void atomicBlockOptimization(TupleSet maxTupleSet, RelationAnalysis ra) {

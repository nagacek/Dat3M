...
for METHOD in ${METHODS[@]}; do
    ...
    for BENCHMARK in ${BENCHMARKS[@]}; do
        ...
        then
            ...
            for i in 1 2 3
            do
                OUTPUT=$(timeout $TIMEOUT java -Xmx4g -jar dartagnan/target/dartagnan-3.0.0.jar cat/$CAT --bound=2 --target=tso --method=$MOPT --solver=$SOLVER $BPL_PATH$BENCHMARK.bpl)
            done
